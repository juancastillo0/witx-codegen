...
impl DartGenerator {
    pub fn new(module_name: Option<String>) -> Self {
        DartGenerator { module_name }
    }
}

impl<T: Write> Generator<T> for DartGenerator {
    fn generate(
        &self,
        writer: &mut T,
...
impl DartGenerator {
    ...
    fn define_as_enum<T: Write>(
        ...
        w.write_line(format!("typedef {} = {};", name.as_type(), repr.as_lang()))?;
        w.eob()?;
        w.write_line(format!("class {} {{", name.as_namespace()))?;
        {
    ...
    fn define_constants_for_type<T: Write>(
        ...
        w.write_line(format!("class {} {{", type_name.as_namespace()))?;
        {
            let mut w = w.new_block();
